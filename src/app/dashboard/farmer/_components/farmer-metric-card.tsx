...
import { Package, PhilippinePeso, ShoppingCart, Star } from "lucide-react"
...
export function FarmerMetricCard({
	...
	return (
		<div
			className={cn(
				"relative overflow-hidden rounded-lg bg-background p-4 text-foreground",
				// gradients[variant],
				className
			)}
		>
			<div className="flex flex-col gap-1">
				...
				{subValue && <p className="text-sm text-foreground">{subValue}</p>}
			</div>
			{/* <div
				className="absolute bottom-0 right-0 h-32 w-32 translate-x-8 translate-y-8 rounded-full border border-foreground/10 bg-foreground/10"
				aria-hidden="true"
			/> */}
			{/* <div className="absolute bottom-2 right-2">
				<Icon className="h-16 w-16 text-foreground/20" />
			</div> */}
		</div>
	)
}
